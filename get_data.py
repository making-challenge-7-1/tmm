...
for date in dayArray:
    for page in range(1, 20):
        ...
        data = requests.get(
            'https://movie.naver.com/movie/sdb/rank/rmovie.naver?sel=pnt&date=' + str(date) + '&tg=' + str(page),
            headers=headers)
        ...
        for movie in movies:
            ...
            if a_tag is not None:
                ...
                data = requests.get(
                    'https://search.naver.com/search.naver?where=nexearch&sm=tab_jum&query=' + title + '정보',
                    headers=headers)
                soup = BeautifulSoup(data.text, 'html.parser')
                try:
